...
async function run() {
  ...
  // Replace the apply manual logs block to include debugging
  procFn = procFn.replace('existing.currentStock += Number(log.qty) || 0;', 'existing.currentStock += Number(log.qty) || 0; console.log("Added. existing is now:", existing);');
  ...
  console.log('Final Result (Channi Rama):', result.find(r => r.partId === '288954409901' && r.location === 'Channi Rama'));
}
run();
