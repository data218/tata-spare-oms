...
async function run() {
  const [inventory, priceList] = await Promise.all([
    fetchTableData('tata_spare_inventory', 'part_no, division, qty, last_receipt, availability'),
  ...
  const now = new Date();
  let validStock = 0, ppniParts = 0, totalPPNIValue = 0;
  ...
  console.log('Stock > 0:', validStock, 'PPNI (>180d):', ppniParts, 'PPNI Value:', totalPPNIValue);
}
run();
